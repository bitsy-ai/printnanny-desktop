...
use {
    super::disks::get_windows_drivenum,
    log::{error, info},
    serde::{Deserialize, Serialize},
    std::path::PathBuf,
    std::process::Command,
...
fn get_boot_mountpoint(disk_path: &str) -> Result<WindowsMount, ImagerError> {
    ...
    while attempts > 0 {
        let cmd = Command::new("powershell.exe")
            .args(["GET-WMIOBJECT", "-query", &query, "|", "ConvertTo-Json"])
            .output()?;
    ...
    while attempts > 0 {
        let diskutil_info_child = Command::new("diskutil")
            ...
            .map_err(|e| ImagerError::CommandIoError {
                msg: e.to_string(),
                cmd: format!("diskutil info -plist {}", &dnode),
            })?;
...
fn _write_bootfile(
    ...
    match std::fs::write(&volume, contents) {
        ...
        Err(e) => Err(ImagerError::BootfileWrite {
            path: volume.display().to_string(),
        }),
    }
}
